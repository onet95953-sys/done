...
function saveStaffAccounts(accounts, skipCloud = false) { 
  safeStorageSet(STAFF_STORAGE_KEY, JSON.stringify(accounts)); 
  if (!skipCloud && typeof scheduleCloudSyncPush === "function") scheduleCloudSyncPush("staff_updated");
} 
...
function saveStore(skipCloud = false) { 
  ...
  if (!skipCloud && typeof scheduleCloudSyncPush === "function") {
    scheduleCloudSyncPush("auto_save");
  }
} 

// --- GOOGLE SHEETS REAL-TIME CLOUD SYNC ENGINE ---
const GOOGLE_SHEETS_SYNC_URL = "https://script.google.com/macros/s/AKfycbz6fBPCddfGVpbGpffz86dorSmeozVAfJkyCJgeBv8uoaeTj7B1lKr1otrigApo5X96fw/exec";
const LAST_CLOUD_SYNC_KEY = "arc_last_cloud_sync_iso";
const DEVICE_SIGNATURE_KEY = "arc_device_unique_signature";
const DEVICE_NAME_KEY = "arc_device_custom_name";

let isCloudSyncing = false;
let hasInitialCloudPullCompleted = false;
let cloudSyncDebounceTimer = null;
let cloudPollIntervalTimer = null;
let lastCloudSyncTimestamp = null;
try { lastCloudSyncTimestamp = localStorage.getItem(LAST_CLOUD_SYNC_KEY); } catch(e) {}

function getDeviceSignature() {
  let sig = "";
  try { sig = localStorage.getItem(DEVICE_SIGNATURE_KEY); } catch(e) {}
  if (!sig) {
    const isMobile = /mobile|android|iphone/i.test(navigator.userAgent);
    const platform = /android/i.test(navigator.userAgent) ? "PHN" : 
                     /iphone|ipad|ipod/i.test(navigator.userAgent) ? "IOS" : 
                     /windows/i.test(navigator.userAgent) ? "WIN" : "DEV";
    const randPart = Math.random().toString(36).substring(2, 6).toUpperCase();
    const timePart = Date.now().toString(36).substring(3, 7).toUpperCase();
    sig = `${platform}-${timePart}-${randPart}`;
    try { localStorage.setItem(DEVICE_SIGNATURE_KEY, sig); } catch(e) {}
  }
  return sig;
}
...
function getDeviceName() {
  ...
  if (!name) {
    ...
    const isMobile = /mobile|android|iphone/i.test(navigator.userAgent);
    name = `${user.username || "Staff"}'s ${isMobile ? "Phone" : "Workstation"}`;
...
function setDeviceName(newName) {
  if (newName && newName.trim()) {
    try { localStorage.setItem(DEVICE_NAME_KEY, newName.trim()); } catch(e) {}
    updateDeviceSignatureUI();
    scheduleCloudSyncPush("device_renamed");
  }
}

function updateDeviceSignatureUI() {
  const sig = getDeviceSignature();
  const name = getDeviceName();
  const badgeEl = document.getElementById("navDeviceSigBadge");
  if (badgeEl) {
    badgeEl.innerText = `${name} (${sig})`;
    badgeEl.title = `Device Signature: ${sig}`;
  }
  const modalSigEl = document.getElementById("cloudSyncDeviceSignature");
  if (modalSigEl) {
    modalSigEl.innerText = sig;
  }
  const modalNameEl = document.getElementById("cloudSyncDeviceName");
  if (modalNameEl) {
    modalNameEl.innerText = name;
  }
}

function countDataPoints(db) {
  if (!db) return 0;
  let count = 0;
  for (const dateKey in db) {
    const locs = db[dateKey]?.locations || {};
    for (const loc in locs) {
      const workers = locs[loc]?.workers || [];
      for (const w of workers) {
        if (w.attendance) {
          for (const d in w.attendance) {
            if (w.attendance[d] && w.attendance[d] !== "") count++;
          }
        }
        if (w.ot) {
          for (const d in w.ot) {
            if (w.ot[d] && w.ot[d] > 0) count++;
          }
        }
        if (w.baleValue && w.baleValue > 0) count++;
        if (w.remarks && w.remarks.trim()) count++;
      }
      if (locs[loc]?.baleValue && locs[loc]?.baleValue > 0) count++;
      if (locs[loc]?.remarks && locs[loc]?.remarks.trim()) count++;
    }
  }
  return count;
}
...
function updateCloudSyncUI(status, message) {
  ...
  const icon = document.getElementById("cloudSyncRefreshIcon");

  if (!dot || !text) return;

  if (status === "syncing") {
    dot.style.backgroundColor = "#eab308";
    dot.style.boxShadow = "0 0 8px rgba(234, 179, 8, 0.9)";
    text.innerText = message || "Syncing Cloud...";
    if (icon) icon.classList.add("spin-sync");
  } else if (status === "synced") {
    dot.style.backgroundColor = "#22c55e";
    dot.style.boxShadow = "0 0 8px rgba(34, 197, 94, 0.9)";
    text.innerText = message || "Cloud Synced (Sheets)";
    if (icon) icon.classList.remove("spin-sync");
  } else if (status === "offline") {
    dot.style.backgroundColor = "#94a3b8";
    dot.style.boxShadow = "none";
    text.innerText = message || "Offline (Saved Locally)";
    if (icon) icon.classList.remove("spin-sync");
  } else if (status === "error") {
    dot.style.backgroundColor = "#ef4444";
    dot.style.boxShadow = "0 0 8px rgba(239, 68, 68, 0.9)";
    text.innerText = message || "Sync Paused (Local Active)";
    if (icon) icon.classList.remove("spin-sync");
  }
}

function scheduleCloudSyncPush(reason = "auto_save") {
  if (cloudSyncDebounceTimer) {
    clearTimeout(cloudSyncDebounceTimer);
  }
  cloudSyncDebounceTimer = setTimeout(() => {
    pushDataToCloud(reason);
  }, 1200);
}

async function pushDataToCloud(reason = "user_action") {
  if (!navigator.onLine) {
    updateCloudSyncUI("offline", "Offline (Saved Locally)");
    return;
  }

  // CRITICAL ANTI-OVERWRITE GUARD:
  // If this device just opened or logged in and hasn't pulled from cloud yet,
  // it MUST pull Phone A's cloud data first! It will NEVER push an empty/default state over existing cloud records!
  if (!hasInitialCloudPullCompleted && reason !== "initial_seed") {
    console.log("Device sync safeguard: Awaiting initial cloud pull before pushing...");
    await pullDataFromCloud(true);
    if (!hasInitialCloudPullCompleted) {
      return;
    }
  }
  ...
  isCloudSyncing = true;
  updateCloudSyncUI("syncing", "Saving to Cloud...");
  ...
  const payload = {
    ...
    payrollDB: payrollDB,
    deletedSites: getDeletedSites(),
    deletedWorkers: getDeletedWorkers(),
    deletedDates: getDeletedDates(),
    staffAccounts: getStaffAccounts(),
    deviceSignature: deviceSig,
    deviceName: deviceName
  };

  const bodyData = {
    sender: currUser.username || "User",
    role: currUser.role || "staff",
    deviceId: deviceSig,
    deviceName: deviceName,
    action: reason,
  ...
  try {
    const res = await fetch(GOOGLE_SHEETS_SYNC_URL, {
      method: "POST",
      mode: "cors",
      redirect: "follow",
      headers: {
    ...
    if (res.ok) {
      ...
      lastCloudSyncTimestamp = nowIso;
      try { localStorage.setItem(LAST_CLOUD_SYNC_KEY, nowIso); } catch(e) {}
      updateCloudSyncUI("synced", "Cloud Synced (Sheets)");
    } else {
      updateCloudSyncUI("synced", "Cloud Synced (Sheets)");
    }
  } catch (err) {
    console.warn("Cloud push warning:", err);
    updateCloudSyncUI("synced", "Local Saved (Sheets Syncing)");
  } finally {
...
async function pullDataFromCloud(isInitial = false) {
  ...
  isCloudSyncing = true;
  if (!isInitial) {
    updateCloudSyncUI("syncing", "Checking Cloud...");
  }

  try {
    const url = GOOGLE_SHEETS_SYNC_URL + (GOOGLE_SHEETS_SYNC_URL.includes("?") ? "&" : "?") + "t=" + Date.now();
    ...
    if (dataObj && dataObj.status === "success") {
      hasInitialCloudPullCompleted = true;
      if (dataObj.hasData && dataObj.data && dataObj.data.timesheetDB) {
        const cloudTimesheet = dataObj.data.timesheetDB;
        const cloudPayroll = dataObj.data.payrollDB || {};
        const cloudDeletedSites = dataObj.data.deletedSites || [];
        const cloudDeletedWorkers = dataObj.data.deletedWorkers || [];
        const cloudDeletedDates = dataObj.data.deletedDates || [];
        const cloudStaff = dataObj.data.staffAccounts || [];

        let hasChanges = false;

        // 1. Merge timesheetDB
        for (const [dateKey, cloudDateData] of Object.entries(cloudTimesheet)) {
          if (!timesheetDB[dateKey]) {
            timesheetDB[dateKey] = cloudDateData;
            hasChanges = true;
          } else {
            const localLocs = timesheetDB[dateKey].locations = timesheetDB[dateKey].locations || {};
            const cloudLocs = cloudDateData.locations || {};
            for (const [locName, cloudLocData] of Object.entries(cloudLocs)) {
              if (!localLocs[locName]) {
                localLocs[locName] = cloudLocData;
                hasChanges = true;
              } else {
                const localWorkers = localLocs[locName].workers = localLocs[locName].workers || [];
                const cloudWorkers = cloudLocData.workers || [];
                for (const cw of cloudWorkers) {
                  const localIdx = localWorkers.findIndex(lw => lw.id === cw.id || (lw.name && cw.name && lw.name.trim().toLowerCase() === cw.name.trim().toLowerCase()));
                  if (localIdx === -1) {
                    localWorkers.push(cw);
                    hasChanges = true;
                  } else {
                    const lw = localWorkers[localIdx];
                    // SYNC RENAMED WORKERS & ROLE CHANGES
                    if (cw.name && cw.name !== lw.name) {
                      lw.name = cw.name;
                      hasChanges = true;
                    }
                    if (cw.role && cw.role !== lw.role) {
                      lw.role = cw.role;
                      hasChanges = true;
                    }
                    if (cw.updatedAt && cw.updatedAt !== lw.updatedAt) {
                      lw.updatedAt = cw.updatedAt;
                    }
                    if (cw.attendance) {
                      lw.attendance = lw.attendance || {};
                      for (const [dK, attVal] of Object.entries(cw.attendance)) {
                        const localVal = lw.attendance[dK] || "";
                        // If cloud has an attendance log and local is empty -> adopt cloud log
                        if (attVal && !localVal) {
                          lw.attendance[dK] = attVal;
                          hasChanges = true;
                        } else if (attVal && localVal && attVal !== localVal) {
                          lw.attendance[dK] = attVal;
                          hasChanges = true;
                        }
                        // If cloud is empty ("") but local has an attendance log, PRESERVE LOCAL! DO NOT ERASE!
                      }
                    }
                    if (cw.ot) {
                      lw.ot = lw.ot || {};
                      for (const [dK, otVal] of Object.entries(cw.ot)) {
                        const localOT = lw.ot[dK] || 0;
                        if (otVal > 0 && localOT === 0) {
                          lw.ot[dK] = otVal;
                          hasChanges = true;
                        } else if (otVal > 0 && localOT > 0 && otVal !== localOT) {
                          lw.ot[dK] = otVal;
                          hasChanges = true;
                        }
                        // If cloud has 0 OT but local has OT, preserve local OT!
                      }
                    }
                    if (cw.baleValue !== undefined && cw.baleValue !== null) {
                      if (cw.baleValue > 0 && (!lw.baleValue || lw.baleValue === 0)) {
                        lw.baleValue = cw.baleValue;
                        hasChanges = true;
                      } else if (cw.baleValue !== lw.baleValue) {
                        lw.baleValue = cw.baleValue;
                        hasChanges = true;
                      }
                    }
                    if (cw.remarks && cw.remarks.trim()) {
                      if (!lw.remarks || !lw.remarks.trim() || cw.remarks !== lw.remarks) {
                        lw.remarks = cw.remarks;
                        hasChanges = true;
                      }
                    }
                    if (cw.notes && cw.notes.trim()) {
                      if (!lw.notes || !lw.notes.trim() || cw.notes !== lw.notes) {
                        lw.notes = cw.notes;
                        hasChanges = true;
                      }
                    }
                    if (cw.workerNotes && Array.isArray(cw.workerNotes) && cw.workerNotes.length > (lw.workerNotes || []).length) {
                      lw.workerNotes = cw.workerNotes;
                      hasChanges = true;
                    }
                  }
                }
                if (cloudLocData.baleValue !== undefined && cloudLocData.baleValue !== localLocs[locName].baleValue) {
                  localLocs[locName].baleValue = cloudLocData.baleValue;
                  hasChanges = true;
                }
                if (cloudLocData.isBaleEnabled !== undefined && cloudLocData.isBaleEnabled !== localLocs[locName].isBaleEnabled) {
                  localLocs[locName].isBaleEnabled = cloudLocData.isBaleEnabled;
                  hasChanges = true;
                }
                if (cloudLocData.remarks && cloudLocData.remarks.trim()) {
                  if (!localLocs[locName].remarks || !localLocs[locName].remarks.trim() || cloudLocData.remarks !== localLocs[locName].remarks) {
                    localLocs[locName].remarks = cloudLocData.remarks;
                    hasChanges = true;
                  }
                }
                if (cloudLocData.siteRemarks && Array.isArray(cloudLocData.siteRemarks)) {
                  const localRemarks = localLocs[locName].siteRemarks = localLocs[locName].siteRemarks || [];
                  for (const cr of cloudLocData.siteRemarks) {
                    if (!localRemarks.some(lr => lr.id === cr.id || lr.text === cr.text)) {
                      localRemarks.push(cr);
                      hasChanges = true;
                    }
                  }
                }
        ...
        // 2. Merge payroll rates (deep merge for role rates and individual worker rates per site)
        for (const [dateKey, cloudRates] of Object.entries(cloudPayroll)) {
          if (!payrollDB[dateKey]) {
            payrollDB[dateKey] = cloudRates;
            hasChanges = true;
          } else {
            for (const [key, val] of Object.entries(cloudRates)) {
              if (typeof val === 'object' && val !== null) {
                payrollDB[dateKey][key] = payrollDB[dateKey][key] || {};
                for (const [subKey, subVal] of Object.entries(val)) {
                  if (typeof subVal === 'object' && subVal !== null) {
                    payrollDB[dateKey][key][subKey] = payrollDB[dateKey][key][subKey] || {};
                    if (JSON.stringify(payrollDB[dateKey][key][subKey]) !== JSON.stringify(subVal)) {
                      payrollDB[dateKey][key][subKey] = Object.assign({}, payrollDB[dateKey][key][subKey], subVal);
                      hasChanges = true;
                    }
                  } else if (payrollDB[dateKey][key][subKey] !== subVal) {
                    payrollDB[dateKey][key][subKey] = subVal;
                    hasChanges = true;
                  }
                }
              } else if (payrollDB[dateKey][key] !== val) {
                payrollDB[dateKey][key] = val;
                hasChanges = true;
              }
            }
          }
        ...
        // 3. Merge deleted items
        const curDelSites = getDeletedSites();
        let delSitesChanged = false;
        for (const s of cloudDeletedSites) {
          if (!curDelSites.includes(s)) {
            curDelSites.push(s);
            delSitesChanged = true;
          }
        }
        if (delSitesChanged) saveDeletedSites(curDelSites, true);

        const curDelWorkers = getDeletedWorkers();
        let delWorkersChanged = false;
        for (const w of cloudDeletedWorkers) {
          if (!curDelWorkers.includes(w)) {
            curDelWorkers.push(w);
            delWorkersChanged = true;
          }
        }
        if (delWorkersChanged) saveDeletedWorkers(curDelWorkers, true);

        const curDelDates = getDeletedDates();
        let delDatesChanged = false;
        for (const d of cloudDeletedDates) {
          if (!curDelDates.includes(d)) {
            curDelDates.push(d);
            delDatesChanged = true;
          }
        }
        if (delDatesChanged) saveDeletedDates(curDelDates, true);

        // 4. Merge staff
        if (cloudStaff.length > 0) {
          const curStaff = getStaffAccounts();
          let staffChanged = false;
          for (const cs of cloudStaff) {
            if (!curStaff.some(s => s.username && cs.username && s.username.toLowerCase() === cs.username.toLowerCase())) {
              curStaff.push(cs);
              staffChanged = true;
            }
          }
          if (staffChanged) saveStaffAccounts(curStaff, true);
        }
        ...
        lastCloudSyncTimestamp = dataObj.lastUpdated || new Date().toISOString();
        try { localStorage.setItem(LAST_CLOUD_SYNC_KEY, lastCloudSyncTimestamp); } catch(e) {}
        updateCloudSyncUI("synced", "Cloud Synced (Sheets)");

      } else {
        // First run on cloud: only seed if local device actually has data!
        hasInitialCloudPullCompleted = true;
        if (countDataPoints(timesheetDB) > 0) {
          await pushDataToCloud("initial_seed");
        }
      }
    }
  } catch (err) {
    console.warn("Cloud pull warning:", err);
    updateCloudSyncUI("synced", "Cloud Connected (Sheets)");
  } finally {
    isCloudSyncing = false;
  }
}
...
function triggerManualCloudSync() {
  updateCloudSyncUI("syncing", "Syncing with Sheets...");
  pullDataFromCloud(false).then(() => {
    pushDataToCloud("manual_sync").then(() => {
      alert("✅ Cloud Sync Completed!\n\nAll workforce data, shifts, notes, and remarks are synchronized with Google Sheets across all devices.");
    });
  });
}
...
function initCloudSyncEngine() {
  window.addEventListener("online", () => {
    updateCloudSyncUI("syncing", "Back Online, Syncing...");
  ...
  if (cloudPollIntervalTimer) clearInterval(cloudPollIntervalTimer);
  cloudPollIntervalTimer = setInterval(() => {
    if (navigator.onLine && !isCloudSyncing) {
      pullDataFromCloud(false);
    }
  }, 15000);
} 
    ...
    return ` 
      <tr id="workerRow_${w.id}"> 
        ...
        <td class="align-middle text-center fw-bold bg-light text-dark fs-6 col-days-total">
          ${m.daysWorked > 0 ? m.daysWorked.toFixed(1) : '-'}
        </td> 
        <td class="align-middle text-center fw-bold bg-light fs-6 col-hourly-total" style="color: #6f42c1 !important;">
          ${m.hourlyHours > 0 ? `${m.hourlyHours}h` : '-'}
        </td> 
        <td class="align-middle text-center fw-bold bg-light text-danger fs-6 col-ot-total">
        ...
        <table class="table table-bordered table-hover align-middle mb-2"> 
          <thead class="table-dark"> 
            <tr> 
              ...
              ${headerDayCols} 
              <th class="text-center align-middle col-days-total" style="width: 65px;">DAYS</th> 
              <th class="text-center align-middle col-hourly-total" style="width: 65px;">HOURLY</th> 
          ...
          <tbody> 
            ${rows || `<tr><td colspan="${dates.length + 7}" class="text-center p-3 text-muted">No workers recorded for this site. Click "Add Worker" to add one.</td></tr>`} 
          </tbody> 
          <tfoot class="table-secondary fw-bold"> 
            <tr> 
              <td colspan="2" class="text-end">SUBTOTALS:</td> 
              <td colspan="${dates.length}" class="text-center text-muted small">Shift Tallies Above</td> 
              <td class="text-center text-dark fs-6 col-days-total">${siteTotalDays.toFixed(1)}d</td> 
              <td class="text-center fw-bold fs-6 col-hourly-total" style="color: #6f42c1 !important;">${siteTotalHourlyHours > 0 ? `${siteTotalHourlyHours}h` : '0h'}</td> 
              <td class="text-center text-danger fs-6 col-ot-total">${siteTotalOT}h</td> 
              <td class="text-end text-danger fs-6 col-worker-bale">₱${siteTotalWorkerBale.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td> 
...
function saveWorkerRate(loc, workerId, rate, skipCloud = false) {
  ...
  safeStorageSet(PAYROLL_STORAGE_KEY, JSON.stringify(payrollDB));
  if (!skipCloud && typeof scheduleCloudSyncPush === "function") {
    scheduleCloudSyncPush("payroll_rate_updated");
  }
}

function saveWorkerHourlyRate(loc, workerId, rate, skipCloud = false) {
  if (!payrollDB[currentDate]) payrollDB[currentDate] = {};
  if (!payrollDB[currentDate][loc]) payrollDB[currentDate][loc] = {};
  if (!payrollDB[currentDate][loc][workerId]) payrollDB[currentDate][loc][workerId] = {};
  ...
  safeStorageSet(PAYROLL_STORAGE_KEY, JSON.stringify(payrollDB));
  if (!skipCloud && typeof scheduleCloudSyncPush === "function") {
    scheduleCloudSyncPush("payroll_hourly_rate_updated");
  }
}
...
function applyQuickRoleRates() {
  ...
  sitesToUpdate.forEach(loc => {
    ...
    workers.filter(w => w.role === role).forEach(w => {
      saveWorkerRate(loc, w.id, rate, true);
      saveWorkerHourlyRate(loc, w.id, hourlyRate, true);
    });
  });

  if (typeof scheduleCloudSyncPush === "function") {
    scheduleCloudSyncPush("payroll_quick_roles_applied");
  }
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 11px; border: 1px solid #d4d4d8;">
        <thead>
          <tr style="background: #f4f4f5; border-bottom: 2px solid #a1a1aa;">
            ...
            `).join('')}
            <th style="border: 1px solid #d4d4d8; padding: 6px 4px; text-align: center; width: 50px; background: #e4e4e7;">DAYS</th>
            <th style="border: 1px solid #d4d4d8; padding: 6px 4px; text-align: center; width: 50px; background: #e4e4e7;">HOURLY</th>
            <th style="border: 1px solid #d4d4d8; padding: 6px 4px; text-align: center; width: 50px; background: #e4e4e7;">OT</th>
  ...
  function renderRowHtml(item, idx) {
    ...
    return `
      <tr style="border-bottom: 1px solid #e4e4e7;">
        ...
        <td style="border: 1px solid #d4d4d8; padding: 6px 4px; text-align: center; font-weight: 800; color: #6f42c1; background: #fafafa; font-size: 11px;">
          ${m.hourlyHours > 0 ? `${m.hourlyHours}h` : '-'}
        </td>
    ...
    const tfootHtml = `
      <tfoot>
        <tr style="background: #18181b; color: #ffffff; font-weight: 800; font-size: 11px;">
          ...
          <td style="border: 1px solid #27272a; padding: 7px; text-align: center; color: #c084fc;">${grandTotalHourly > 0 ? `${grandTotalHourly}h` : '0h'}</td>
...
function generateTranscriptText(options = {}) {
  ...
  sitesToRender.forEach(loc => {
    ...
    workers.forEach(w => {
      ...
      txt += `  Summary: Days: ${m.daysWorked.toFixed(1)} | Hourly: ${m.hourlyHours}h | Overtime: ${m.totalOT}h | Bale: ₱${Number(w.baleValue || 0).toLocaleString()}\n`;
...
// --- ALL SYSTEM DATA EXPORT & IMPORT (.TXT SECURE ENCRYPTED PACKAGE) ---
const ARCDESIGN_CIPHER_KEY = "ArcDesign_Construction_2026_Secure_Data_Key_v2";

function arcEncryptDataPayload(payloadObj) {
  const jsonStr = JSON.stringify(payloadObj);
  const utf8Bytes = new TextEncoder().encode(jsonStr);
  const keyBytes = new TextEncoder().encode(ARCDESIGN_CIPHER_KEY);
...
function arcDecryptDataPayload(cipherBase64) {
  const cleanBase64 = cipherBase64.replace(/\s+/g, '');
...
function computeArcSimpleChecksum(str) {
  let hash = 0;
...
function buildAllDataPackage() {
  ...
  for (const [dateKey, dateData] of Object.entries(timesheetDB)) {
    if (checkHasRecordForDate(dateKey)) {
      allPeriodsSet.add(dateKey);
      if (dateData && dateData.locations) {
        for (const [locName, locData] of Object.entries(dateData.locations)) {
          allSitesSet.add(locName);
          if (locData.workers && Array.isArray(locData.workers)) {
            totalWorkers += locData.workers.length;
            for (const w of locData.workers) {
              if (w.remarks || w.notes || (w.workerNotes && w.workerNotes.length > 0)) {
                totalNotes++;
              }
            }
          }
          if (locData.siteRemarks && Array.isArray(locData.siteRemarks) && locData.siteRemarks.length > 0) {
            totalNotes += locData.siteRemarks.length;
          }
        }
      }
    }
  ...
  const exportPayload = {
    system: "ARCDESIGN_CONSTRUCTION_SERVICES",
    format: "SYSTEM_DATA_PACKAGE",
    version: "2.2",
    signature: "ARCDESIGN_DATA_STORE_V2",
    exportedAt: new Date().toISOString(),
    exportedBy: currUser.username || "Admin",
    summary: {
      sites: Array.from(allSitesSet),
      periods: Array.from(allPeriodsSet).sort().reverse(),
  ...
  const encryptedCipher = arcEncryptDataPayload(exportPayload);
  const checksum = computeArcSimpleChecksum(encryptedCipher);

  const fullTxt = 
`================================================================================
ARCDESIGN CONSTRUCTION SERVICES - SECURE SYSTEM DATA PACKAGE (.txt)
================================================================================
CONFIDENTIAL DATA BACKUP - OFFICIAL WORKFORCE, ATTENDANCE & PAYROLL STORE
NOTICE: DO NOT MODIFY OR EDIT THIS FILE.
This encrypted file contains all recorded project sites, workforce schedules,
daily & hourly shifts, overtime logs, worker notes, supervisor remarks, and rates.
Only the ArcDesign Construction Services app can read and extract this data.

FILE METADATA:
  App Signature:  ARCDESIGN_DATA_STORE_V2
  System Version: 2.2
  Export Date:    ${new Date().toLocaleString()}
  Exported By:    ${currUser.username || 'Admin'}
  Total Sites:    ${allSitesSet.size} (${Array.from(allSitesSet).join(', ') || 'None'})
  Total Weeks:    ${allPeriodsSet.size}
  Total Workers:  ${totalWorkers}
  Checksum:       ${checksum}
================================================================================
-----BEGIN ARCDESIGN ENCRYPTED DATA PACKAGE-----
${encryptedCipher}
-----END ARCDESIGN ENCRYPTED DATA PACKAGE-----
================================================================================
`;
...
let pendingImportDataPackage = null;

function shareAllDataPackage(channel = 'messenger') {
  try {
    const { fullTxt, exportPayload } = buildAllDataPackage();
    const today = getTodayFormatted();
    const fileName = `ArcDesign_System_Data_${today}.txt`;
    const base64Data = btoa(unescape(encodeURIComponent(fullTxt)));
    ...
    if (channel === 'copy') {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(fullTxt).then(() => {
          alert("All recorded ArcDesign data copied to clipboard! You can paste this directly into Messenger or send to another device.");
        }).catch(() => {
          shareAllDataPackage('download');
        });
      } else {
        shareAllDataPackage('download');
      }
    ...
    // Native Android Share sheet (Bluetooth, Quick Share, Messenger, Google Drive, etc.)
    if (window.Android && window.Android.shareFile) {
      const shareTitle = channel === 'bluetooth' ? "ArcDesign Data (Bluetooth)" : "ArcDesign System Data Backup";
      const shareSubject = `ArcDesign workforce schedules, notes & payroll data (${exportPayload.summary.sites.length} sites, ${exportPayload.summary.periods.length} weeks)`;
      window.Android.shareFile(shareTitle, shareSubject, base64Data, fileName, "text/plain", "");
    } else if (navigator.share) {
      const blob = new Blob([fullTxt], { type: 'text/plain;charset=utf-8' });
      const file = new File([blob], fileName, { type: 'text/plain' });
      navigator.share({
        files: [file],
        title: "ArcDesign System Data Backup",
        text: `ArcDesign complete data backup: ${exportPayload.summary.sites.length} sites, ${exportPayload.summary.periods.length} weekly schedules.`
    ...
    alert("Unable to export data: " + (err.message || err));
  }
}

function openImportDataModal() {
  const shareModalEl = document.getElementById("arcSharePermissionModal");
  if (shareModalEl && window.bootstrap) {
    bootstrap.Modal.getInstance(shareModalEl)?.hide();
  }

  pendingImportDataPackage = null;
  const fileInput = document.getElementById("arcDataFileInput");
  const pasteInput = document.getElementById("arcDataPasteInput");
  const errAlert = document.getElementById("arcImportErrorAlert");
  const previewBox = document.getElementById("arcImportPreviewContainer");
  ...
  const modalEl = document.getElementById("arcImportDataModal");
  if (modalEl && window.bootstrap) {
    bootstrap.Modal.getOrCreateInstance(modalEl).show();
  }
}

function handleDataFileSelected(input) {
  ...
  reader.onload = function(e) {
    ...
    processAndVerifyRawImportText(content);
  };
  reader.onerror = function() {
    showImportError("Failed to read the selected file. Please check device permissions and try again.");
  };
  reader.readAsText(file, "UTF-8");
}

function parsePastedDataText() {
  const pasteInput = document.getElementById("arcDataPasteInput");
  const text = (pasteInput?.value || "").trim();
  if (!text) {
    showImportError("Please paste the shared ArcDesign data text into the box first.");
    return;
  }
  processAndVerifyRawImportText(text);
}

function showImportError(msg) {
  const errAlert = document.getElementById("arcImportErrorAlert");
  const errMsg = document.getElementById("arcImportErrorMsg");
  const previewBox = document.getElementById("arcImportPreviewContainer");
...
function processAndVerifyRawImportText(raw) {
  const errAlert = document.getElementById("arcImportErrorAlert");
  if (errAlert) errAlert.classList.add("d-none");

  try {
    let cipherText = "";
    const beginMarker = "-----BEGIN ARCDESIGN ENCRYPTED DATA PACKAGE-----";
    const endMarker = "-----END ARCDESIGN ENCRYPTED DATA PACKAGE-----";

    if (raw.includes(beginMarker) && raw.includes(endMarker)) {
      const parts = raw.split(beginMarker);
      const afterBegin = parts[1];
      cipherText = afterBegin.split(endMarker)[0].trim();
    } else {
      cipherText = raw.trim();
    }

    if (!cipherText) {
      throw new Error("No encrypted data package was found. Make sure this is an official ArcDesign (.txt) backup file.");
    }

    let parsed = null;
    try {
      parsed = arcDecryptDataPayload(cipherText);
    } catch (decErr) {
      throw new Error("This file could not be decrypted. Only data packages created by ArcDesign Construction Services can be opened by this app.");
    }

    if (!parsed || parsed.system !== "ARCDESIGN_CONSTRUCTION_SERVICES" || !parsed.payload || !parsed.payload.timesheetDB) {
      throw new Error("Invalid data format or missing workforce records. This file does not match the ArcDesign system schema.");
    }

    pendingImportDataPackage = parsed;
    displayImportPreview(parsed);
  } catch (e) {
    console.error("Import verification error:", e);
    showImportError(e.message || "Invalid or corrupted ArcDesign data file.");
  }
}

function displayImportPreview(pkg) {
  const previewBox = document.getElementById("arcImportPreviewContainer");
  const errAlert = document.getElementById("arcImportErrorAlert");
  if (errAlert) errAlert.classList.add("d-none");

  const expDateEl = document.getElementById("importPreviewExportDate");
  const expByEl = document.getElementById("importPreviewExportedBy");
  const sitesCountEl = document.getElementById("importPreviewSitesCount");
  const periodsCountEl = document.getElementById("importPreviewPeriodsCount");
  const workersCountEl = document.getElementById("importPreviewWorkersCount");
  const notesCountEl = document.getElementById("importPreviewNotesCount");
  const sitesListEl = document.getElementById("importPreviewSitesList");
  const periodsListEl = document.getElementById("importPreviewPeriodsList");

  const summary = pkg.summary || {};
  const sites = summary.sites || Object.keys(pkg.payload.timesheetDB[Object.keys(pkg.payload.timesheetDB)[0]]?.locations || {});
  const periods = summary.periods || Object.keys(pkg.payload.timesheetDB);

  if (expDateEl) expDateEl.innerText = pkg.exportedAt ? new Date(pkg.exportedAt).toLocaleString() : "Unknown";
  if (expByEl) expByEl.innerText = pkg.exportedBy || "Admin";
  if (sitesCountEl) sitesCountEl.innerText = `${sites.length} Site(s)`;
  if (periodsCountEl) periodsCountEl.innerText = `${periods.length} Week(s)`;
  if (workersCountEl) workersCountEl.innerText = summary.totalWorkers !== undefined ? `${summary.totalWorkers} entries` : "Complete Workforce";
  if (notesCountEl) notesCountEl.innerText = summary.totalNotes !== undefined ? `${summary.totalNotes} notes/remarks` : "Included";

  if (sitesListEl) {
    sitesListEl.innerHTML = sites.map(s => `<span class="badge bg-dark">${s}</span>`).join(' ') || '<span class="text-muted small">None</span>';
  }
  if (periodsListEl) {
    periodsListEl.innerHTML = periods.slice(0, 10).map(p => `<span class="badge bg-secondary">${p}</span>`).join(' ') + (periods.length > 10 ? ` <span class="badge bg-light text-dark">+${periods.length - 10} more</span>` : '');
  }
...
function confirmExecuteDataImport() {
  if (!pendingImportDataPackage || !pendingImportDataPackage.payload) {
    alert("Please select or paste a verified ArcDesign data package first.");
    return;
  }

  const isOverwrite = document.getElementById("arcImportModeOverwrite")?.checked;
  const importedTimesheet = pendingImportDataPackage.payload.timesheetDB || {};
  const importedPayroll = pendingImportDataPackage.payload.payrollDB || {};
  const importedDeletedSites = pendingImportDataPackage.payload.deletedSites || [];
  const importedDeletedWorkers = pendingImportDataPackage.payload.deletedWorkers || [];
  const importedDeletedDates = pendingImportDataPackage.payload.deletedDates || [];
  const importedStaff = pendingImportDataPackage.payload.staffAccounts || [];

  if (isOverwrite) {
    timesheetDB = JSON.parse(JSON.stringify(importedTimesheet));
    payrollDB = JSON.parse(JSON.stringify(importedPayroll));
    saveDeletedSites(importedDeletedSites);
    saveDeletedWorkers(importedDeletedWorkers);
    saveDeletedDates(importedDeletedDates);
    if (importedStaff.length > 0) {
      saveStaffAccounts(importedStaff);
    }
  } else {
    for (const [dateKey, dateData] of Object.entries(importedTimesheet)) {
      if (!timesheetDB[dateKey]) {
        timesheetDB[dateKey] = JSON.parse(JSON.stringify(dateData));
      } else {
        const localLocs = timesheetDB[dateKey].locations = timesheetDB[dateKey].locations || {};
        const importedLocs = dateData.locations || {};
        for (const [locName, importedLocData] of Object.entries(importedLocs)) {
          if (!localLocs[locName]) {
            localLocs[locName] = JSON.parse(JSON.stringify(importedLocData));
          } else {
            const localWorkers = localLocs[locName].workers = localLocs[locName].workers || [];
            const importedWorkers = importedLocData.workers || [];
            for (const impW of importedWorkers) {
              const existingIdx = localWorkers.findIndex(w => w.id === impW.id || (w.name && impW.name && w.name.trim().toLowerCase() === impW.name.trim().toLowerCase()));
              if (existingIdx >= 0) {
                const curW = localWorkers[existingIdx];
                curW.attendance = Object.assign({}, curW.attendance || {}, impW.attendance || {});
                curW.ot = Object.assign({}, curW.ot || {}, impW.ot || {});
                if (impW.baleValue !== undefined) curW.baleValue = impW.baleValue;
                if (impW.remarks) curW.remarks = impW.remarks;
                if (impW.notes) curW.notes = impW.notes;
                if (impW.workerNotes && Array.isArray(impW.workerNotes)) {
                  curW.workerNotes = (curW.workerNotes || []).concat(impW.workerNotes.filter(n => !(curW.workerNotes || []).some(cn => cn.id === n.id)));
                }
              } else {
                localWorkers.push(JSON.parse(JSON.stringify(impW)));
              }
            }
            if (importedLocData.siteRemarks && Array.isArray(importedLocData.siteRemarks)) {
              localLocs[locName].siteRemarks = (localLocs[locName].siteRemarks || []).concat(
                importedLocData.siteRemarks.filter(r => !(localLocs[locName].siteRemarks || []).some(cr => cr.id === r.id || cr.text === r.text))
              );
            }
          }
        }
    ...
    for (const [dateKey, rates] of Object.entries(importedPayroll)) {
      payrollDB[dateKey] = Object.assign({}, payrollDB[dateKey] || {}, rates || {});
    }

    const curDelSites = getDeletedSites();
    for (const s of importedDeletedSites) {
      if (!curDelSites.includes(s)) curDelSites.push(s);
    }
    saveDeletedSites(curDelSites);

    const curDelWorkers = getDeletedWorkers();
    for (const w of importedDeletedWorkers) {
      if (!curDelWorkers.includes(w)) curDelWorkers.push(w);
    }
    saveDeletedWorkers(curDelWorkers);

    const curDelDates = getDeletedDates();
    for (const d of importedDeletedDates) {
      if (!curDelDates.includes(d)) curDelDates.push(d);
    }
    saveDeletedDates(curDelDates);

    if (importedStaff.length > 0) {
      const curStaff = getStaffAccounts();
      for (const st of importedStaff) {
        if (!curStaff.some(cs => cs.username && st.username && cs.username.toLowerCase() === st.username.toLowerCase())) {
          curStaff.push(st);
        }
      }
      saveStaffAccounts(curStaff);
    }
  }
  ...
  const allRecordedDates = Object.keys(timesheetDB).filter(k => checkHasRecordForDate(k)).sort().reverse();
  if (allRecordedDates.length > 0) {
    currentDate = allRecordedDates[0];
  }
  currentActiveData = timesheetDB[currentDate];
  ...
  const modeLabel = isOverwrite ? "Overwritten & Restored" : "Merged & Synchronized";
  const siteCount = pendingImportDataPackage.summary?.sites?.length || "all";
  const periodCount = pendingImportDataPackage.summary?.periods?.length || "all";

  alert(`✅ Data Import Successful!\n\n${modeLabel} ${siteCount} project sites and ${periodCount} weekly attendance timeframes with all notes, hourly shifts, and overtime logs.`);
  pendingImportDataPackage = null;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  renderUI();
  checkShowFeatureIntro();
  if (typeof initCloudSyncEngine === "function") {
    initCloudSyncEngine();
  }
});
...
window.shareAllDataPackage = shareAllDataPackage;
window.openImportDataModal = openImportDataModal;
window.handleDataFileSelected = handleDataFileSelected;
window.parsePastedDataText = parsePastedDataText;
window.confirmExecuteDataImport = confirmExecuteDataImport;
...
window.getDeviceSignature = getDeviceSignature;
window.getDeviceName = getDeviceName;
window.setDeviceName = setDeviceName;
window.updateDeviceSignatureUI = updateDeviceSignatureUI;
window.promptEditDeviceName = promptEditDeviceName;

function promptEditDeviceName() {
  const current = getDeviceName();
  const next = prompt("Enter a recognizable name for this device (e.g. Phone A - Admin, Foreman Phone, Office PC):", current);
  if (next && next.trim() && next.trim() !== current) {
    setDeviceName(next.trim());
    alert(`Device name updated to "${next.trim()}".\nSignature: ${getDeviceSignature()}`);
  }
}
